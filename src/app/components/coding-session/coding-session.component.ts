...
import { Character } from "../../../shared/testFile/character.model";
@Component({
    selector: 'c-coding',
    styleUrl: './coding-session.component.scss',
    templateUrl: './coding-session.component.html',
})

export class CodingSession implements OnInit{
    protected heroes: Array<Character>;
    protected filteredList: Array<Character>;
    constructor() {
        this.heroes = [];
        this.filteredList = [];
    }

    protected filterCharacters = (ev: any) => {
        this.filteredList = this.heroes.filter(x => x.name.toLocaleLowerCase().includes(ev.target.value.toLowerCase()));
    };
